...
exports.getMR_DiagnosisType = async (req, res) => {
    try {
        ...
        const countResult = await countRequest.query(
            `SELECT COUNT(*) as total FROM MR_DiagnosisType ${filterClause}`
        );
...
exports.getMR_DiagnosisTypeById = async (req, res) => {
    try {
        const { id } = req.params;
        const pool = req.db;
        if (!pool) {
        ...
        const result = await pool
            .request()
            .input('id', sql.Int, id)
            .query('SELECT * FROM MR_DiagnosisType WHERE MR_ID = @id');

        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'MR_DiagnosisType not found' });
        }
